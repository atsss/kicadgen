...
def validate_component(spec: ComponentSpec) -> ValidationReport:
    ...
    if spec.footprint.pitch_mm < 0.2:
        report.errors.append(f"Invalid pitch: {spec.footprint.pitch_mm}mm < 0.2mm minimum")

    # Rule 3: Pad width constraint
    if spec.footprint.pad_width_mm > spec.footprint.pitch_mm:
        report.errors.append(
            f"Invalid pad width: {spec.footprint.pad_width_mm}mm > pitch {spec.footprint.pitch_mm}mm"
        )
    ...
    # Rule 5: Unit heuristics for mil/inch detection
    # Suspected mils (values like 10-250 that would be tiny in mm)
    if 10 <= spec.footprint.pitch_mm <= 250 and spec.footprint.pitch_mm > spec.footprint.body_length_mm:
